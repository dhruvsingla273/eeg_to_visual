...
#                 align_corners=False,
#             ).squeeze()
#         o=prediction.detach().cpu().numpy()
        
#         o = cv2.resize(o,(64,64))
#         o = (o - np.min(o))/(np.max(o)-np.min(o))
        
#         prediction = np.array([np.stack([o,o,o,o], axis=0)])
        
#         image_raw = self.processor(images=image_raw, return_tensors="pt")
#         image_raw['pixel_values'] = image_raw['pixel_values'].squeeze(0)
# #         print(image_raw.keys())
# #         
        
#         return {'eeg': eeg, 'label': label, 'image': self.image_transform(image), 'image_raw': image_raw,'depth_map':prediction}
#         # Return
#         # return eeg, label
